...
func (r *Resource) decodeWritePayloadFor(view *resolvedResource, ctx *ninja.Context, mode fieldMode) (map[string]any, error) {
	...
	for name, raw := range payload {
		...
		if !ok {
			return nil, ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, fmt.Sprintf("unknown field %q", name))
		}
		if !view.allowed(field, mode) {
			return nil, ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, fmt.Sprintf("field %q is not writable", name))
		}
		decoded, err := field.decodeJSON(raw)
		if err != nil {
			return nil, ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, fmt.Sprintf("field %q: %s", name, err.Error()))
		}
		values[name] = decoded
	}
	return values, nil
...
func (r *Resource) validateRequiredFor(view *resolvedResource, values map[string]any, mode fieldMode) error {
	...
	for _, field := range view.fields {
		...
		return ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, fmt.Sprintf("field %q is required", meta.Name))
	}
	return nil
}
...
func (r *Resource) applyListQueryFor(view *resolvedResource, db *gorm.DB, query url.Values, in *listInput) (*gorm.DB, error) {
	if term := strings.TrimSpace(in.Search); term != "" {
		if len(view.metadata.SearchFields) == 0 {
			return nil, ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, "search is not enabled for this resource")
		}
	...
	if strings.TrimSpace(in.Sort) != "" {
		...
		for _, sortField := range order.ParseSort(in.Sort) {
			...
			if field == nil {
				return nil, ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, fmt.Sprintf("unsupported sort field %q", sortField.Name))
			}
			direction := "ASC"
			if sortField.Desc {
				direction = "DESC"
...
func (r *Resource) handleRelationOptions(site *Site) func(*ninja.Context, *relationOptionsInput) (*RelationOptionsOutput, error) {
	return func(ctx *ninja.Context, in *relationOptionsInput) (*RelationOptionsOutput, error) {
		...
		if target == nil {
			return nil, ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, fmt.Sprintf("relation resource %q is not registered", fieldMeta.Relation.Resource))
		}
		...
		if valueField == nil || labelField == nil {
			return nil, ninja.NewErrorWithCode(http.StatusBadRequest, http.StatusBadRequest, fmt.Sprintf("relation fields %q/%q are not available", fieldMeta.Relation.ValueField, fieldMeta.Relation.LabelField))
		}
