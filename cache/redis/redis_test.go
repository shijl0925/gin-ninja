...
func TestRedisCacheStoreAdditionalCoverage(t *testing.T) {
	...
	t.Run("get set delete and invalid data", func(t *testing.T) {
		...
		store.SetContext(context.Background(), "expired-on-set", &ninja.CachedResponse{Expires: time.Now().Add(-time.Second)})
...
func TestRedisCacheStoreGetTagInvalidateAndLock(t *testing.T) {
	mr := miniredis.RunT(t)
	store, err := NewRedisCacheStore(RedisCacheConfig{
		Addr:   mr.Addr(),
		Prefix: "test:",
	})
	if err != nil {
...
func TestRedisCacheStoreTagMetadataFollowsCacheTTL(t *testing.T) {
	redisServer := miniredis.RunT(t)
	...
	store.Set("long", &ninja.CachedResponse{Status: http.StatusOK, Expires: time.Now().Add(time.Minute)})
	store.AddTags("long", "users")
	store.Set("short", &ninja.CachedResponse{Status: http.StatusOK, Expires: time.Now().Add(time.Second)})
