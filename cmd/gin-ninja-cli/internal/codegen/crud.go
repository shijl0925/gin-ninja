...
// WriteCRUDTestFile generates a unit-test skeleton for the configured CRUD scaffold.
func WriteCRUDTestFile(cfg CRUDConfig, outputPath string) error {
	content, err := GenerateCRUDTest(cfg)
	if err != nil {
		return err
	}
	if err := os.MkdirAll(filepath.Dir(outputPath), 0o700); err != nil {
	...
	if err := os.WriteFile(outputPath, content, 0o600); err != nil {
		return fmt.Errorf("write generated test file: %w", err)
	}
	return nil
}
...
// GenerateCRUDTest returns formatted Go source for a CRUD unit-test skeleton.
func GenerateCRUDTest(cfg CRUDConfig) ([]byte, error) {
	model, err := loadModelSpec(cfg)
	if err != nil {
		return nil, err
	}
	...
	payload := buildTemplateData(model)
	var raw bytes.Buffer
	if err := crudTestTemplate.Execute(&raw, payload); err != nil {
		return nil, fmt.Errorf("execute test template: %w", err)
	}
	formatted, err := format.Source(raw.Bytes())
	if err != nil {
		return nil, fmt.Errorf("format generated test source: %w", err)
	}
	return formatted, nil
}
...
// DefaultTestOutputName returns the default unit-test skeleton file name for a model.
func DefaultTestOutputName(model string) string {
	return strings.TrimSuffix(DefaultOutputName(model), ".go") + "_test.go"
}

// TestOutputPath returns the unit-test skeleton path paired with a CRUD output path.
func TestOutputPath(outputPath string) string {
	if strings.HasSuffix(outputPath, ".go") {
		return strings.TrimSuffix(outputPath, ".go") + "_test.go"
	}
	return outputPath + "_test.go"
}
...
func load{{ $.ModelName }}{{ .FieldName }}Relations(db *gorm.DB, ids []{{ .TargetIDType }}) ([]{{ .TargetModel }}, error) {
	...
	for _, id := range ids {
		...
		item, ok := byID[id]
		if !ok {
			return nil, ninja.NewError(400, fmt.Sprintf("relation %q record %v not found", "{{ .FieldName }}", id))
		}
		out = append(out, item)
	}
	return out, nil
...
func sync{{ $.ModelName }}{{ .FieldName }}Relation(db *gorm.DB, item *{{ $.ModelName }}, id {{ .TargetIDType }}) error {
	...
	if err := db.Where("{{ .TargetIDColumn }} = ?", id).First(&related).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ninja.NewError(400, fmt.Sprintf("relation %q record %v not found", "{{ .FieldName }}", id))
		}
		return err
	}
	return db.Model(item).Association("{{ .FieldName }}").Replace(&related)
...
var crudTestTemplate = template.Must(template.New("crud-test").Parse(`// Code generated by gin-ninja generate crud -with-tests; DO NOT EDIT.
