...
func (u *User) BeforeSave(*gorm.DB) error {
	...
	if len(u.Password) < 8 {
		return ninja.NewErrorWithCode(400, 400, "field \"password\" must be at least 8 characters")
	}
...
func syncUserRoles(tx *gorm.DB, user *User, roleIDs []uint) error {
	...
	for _, id := range roleIDs {
		if id == 0 {
			return ninja.NewErrorWithCode(400, 400, "field \"role_ids\" must not contain zero")
		}
	...
	for _, id := range normalized {
		role, ok := roleByID[id]
		if !ok {
			return ninja.NewErrorWithCode(400, 400, fmt.Sprintf("role %d does not exist", id))
		}
